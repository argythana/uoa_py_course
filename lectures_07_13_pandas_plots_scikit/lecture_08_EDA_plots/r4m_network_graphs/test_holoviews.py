...
hv_nodes = hv.Nodes(nodes_df, kdims=["index", "user_id", "dummy"]).sort()
...
# Create a layout dictionary for StaticLayoutProvider using numeric indices
layout = {
    i: (float(i), float(i)) for i in range(len(nodes_df))
}  # Example layout, replace with actual coordinates

# Access the Bokeh plot object and set the layout provider
bokeh_plot = hv.render(hv_graph)
bokeh_plot.layout_provider = StaticLayoutProvider(graph_layout=layout)
