...
description: >
  ...
  the UoA "Python for Data Science, ML and AI" MSc course. Trigger on "draft the
  next linkedin post", "write the linkedin post about <theme>", "linkedin post
  for the course updates", "continue the course post series", "mark post N as
  published", or "rebuild the linkedin series plan". It reads the series plan in
  admin_docs/linkedin_posts/series_plan.md, verifies evidence in git history and
  repo files, writes one plain-text draft per invocation to
  admin_docs/linkedin_posts/post_NN_<slug>.md, and updates the plan's status
  column. It never publishes (the maintainer posts manually), never invents
  numbers or features not verifiable in the public repo, and never mentions
  students, grades, PII, secrets, or gitignored content. Do NOT use it to write
  feedback or grades, edit lecture material, or post to any platform. One post
  per invocation unless the user asks for the whole series.
---
